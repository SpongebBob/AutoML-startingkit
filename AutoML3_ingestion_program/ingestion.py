...
if __name__=="__main__" and debug_mode<4:
    ...
    time_info = [overall_start,0] # time info avaiable for models
    ...
    for basename in datanames: # Loop over datasets
        ...
        # Starting time when processing the data set
	...
	# predict and re train the model
        ...
        for k in range(n_testbatches):
                i=k+1            
		#try:
                E = DataManager(basename, input_dir, replace_missing=False, max_samples=max_samples, verbose=verbose,testdata=i,ltl=0)                
                Y_pred = M.predict(E.data[0],D.info,time_info)
		...
		## Do not fit the last batch		
                if i<n_testbatches:
                        uE=E
                        E = DataManager(basename, hidden_dir, replace_missing=False, max_samples=max_samples, verbose=verbose,testdata=i,ltl=1)	
                        M.fit(uE.data[0], E.label[0],D.info,time_info) 
                        vprint( verbose,  "[+] Fitting success, time spent so far %5.2f sec" % (time.time() - start))
